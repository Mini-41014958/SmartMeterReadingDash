    ...
    public class ReportGenService
    {
        ...
        public async Task<DashboardReport> GenerateReportAsync( string readingMonth,UserAccessScope userScope)
        {
            ...
            try
            {
                ...
                {
                    throw new UnauthorizedAccessException( $"Invalid company scope: {company}");
                }
                ...
                CreateDashboardSheet( workbook, readingMonth,  meterSummary, meterSummaryBypl,  brplReceived,  byplReceived, userScope);
        ...
        private void CreateDashboardSheet( XLWorkbook workbook, string readingMonth, TotalMeterSummary? brplMeter,  TotalMeterSummaryBypl? byplMeter, 
            MeterReceivedSummary? brplReceived, List<MeterReceivedSummaryBypl>? byplReceived,UserAccessScope scope)
        {
            ...
            ws.Range("B2:K4")
                .Style
                .Border
                .BottomBorder = XLBorderStyleValues.Medium;

            ws.Range("B2:K4")
                .Style
                .Border
                .BottomBorderColor = Palette.Blue;
            ...
            int departmentCompanyHeaderRow =
                departmentSectionRow + 1;


            if (showBrpl)
            {
                WriteDepartmentCompanyHeader(
                    ws,
                    departmentCompanyHeaderRow,
                    2,
                    6,
                    "BRPL",
                    Palette.Blue);
            ...
            int departmentHeaderRow =
                departmentCompanyHeaderRow + 1;
            ...
            if (showBrpl)
            {
                foreach (var item in brplDepartment)
                {
                    ...
                    ws.Cell(brplDeptRow, 3).Value =
                        item.HesDownload;

                    ws.Cell(brplDeptRow, 4).Value =
                        item.Failed;
            ...
            if (showBypl)
            {
                foreach (var item in byplDepartment)
                {
                    ...
                    ws.Cell(byplDeptRow, 8).Value =
                        item.HesDownload;

                    ws.Cell(byplDeptRow, 9).Value =
                        item.Failed;


                    ws.Cell(byplDeptRow, 10).Value =
                        total > 0
                            ? (double)item.HesDownload / total
                            : 0;


                    ws.Cell(byplDeptRow, 11).Value =
                        total > 0
                            ? (double)item.Failed / total
                            : 0;
            ...
            {
                foreach (var item in
                    brplFailureReasons.OrderByDescending(
                        x => x.count ?? 0))
                {
                    int count =
                        item.count ?? 0;
            ...
            int byplFailureTotal =
                showBypl
                    ? byplFailureReasons.Sum(
                        x => x.count ?? 0)
                    : 0;
            ...
            {
                foreach (var item in
                    byplFailureReasons.OrderByDescending(
                        x => x.count ?? 0))
                {
                    int count =
                        item.count ?? 0;
            ...
            ws.Cell(
                footerRow,
                2).Value =
                $"{CompanySummaryLabel(isSuperAdmin, company)}  |  " +
        ...
        private static void CreateBrplFailedSheet(
            XLWorkbook workbook,
            List<MeterDownloadDetailedSummary> data)
        {
            var ws =
                workbook.Worksheets.Add(
                    "BRPL Failed");
